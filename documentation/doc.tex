...
\begin{Definice}
Nech't $G$ je graf a $M$ nějaké párování v něj. Necht' $u$ je vrchol grafu $G$ nepokrytý párováním $M$. Tehdy \textit{$M$-alternující $u$-strom} je to strom $T$ s kořenem ve vrcholu $u$ v grafu $G$ takový, že pro libovolný uzel $v\in V(G)$ cesta $uTv$ je alternující cesta. 
\end{Definice}

\begin{Definice}
$M$-alternující $u$-strom je \textit{$M$-pokrytý $u$-strom} jestli párování $M \cap E(T)$  pokrývá všechny vrcholy stromu $T$ kromě vrcholu $u$.
\end{Definice}
...
INPUT:Nechť $G(X,Y)$ je bipartitní graf. \\
OUTPUT: maximální párování.\\
\begin{enumerate}
...
\subsection{Algoritm Augmenting Path Search}
INPUT: graf $G$, párování $M$, nepotrytý vrchol $u$.\\
OUTPUT:APS-trom $T$ anebo párování $M$ zvětšené o jednu hranu. \\
\begin{enumerate}
\item $V(T)={u}$, $E(T)=\emptyset$,$R(T)={u}$
\item While $\exists xy$ , že $x \in R(T)$ a $y \notin V(T)$.
	\begin{enumerate}
	\item $V(T) = V(T)\cup {u}$
	\item $E(T) = E(T)\cup {xy}$
	\item Jestli ${y}$ pokryt párováním $M$
		\begin{enumerate}
		\item $M=M\div E(P)$, kde $P=uTy$
		\item Vratit $M$
		\end{enumerate}
	\item Jestli ${y}$ nepokryt párováním $M$
		\begin{enumerate}
		\item Tehdy pro každé $z$, takové že $yz \in M$
		\item $V(T) = V(T)\cup {z}$
		\item $E(T) = E(T)\cup {yz}$
		\item $R(T) = R(T)\cup{z}$
		\end{enumerate}
	\end{enumerate}
\item return $T=(V(T),E(T))$
\end{enumerate}
